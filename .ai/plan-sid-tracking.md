...
## S5.12 (queued 2026-09-24 09:38, Morten: "Let's do 3, chip comparisons"): chip-comparison follow-up fixes authorized
Fix the ranked REAL DIVERGENCES from .ai/sid-chip-comparison-report.md — #2 combined waveforms (+6..+20 dB, reSID wedge/alternation semantics), #3 filter ceiling 4 kHz (GT2 default), #4 6581 cutoff curve (2-3x off, missing $80 step), #5 tick-0 effects remainder. #1 gate bit already fixed (S5.9). Licence discipline: reSID combination tables/cutoff curve are GPL — derive from semantics + published measurement facts with citations, disclose derived values, never embed GPL bytes. Explicitly out of scope (recorded): #6 new-note first frame (needs own batch, interacts with S5.6 timing), #7 sampling (deliberate), #8-#11 lower-impact items. Spawn after S5.11 (instruments) lands. jt_letgo red-first, no regressions to S5.9/S5.6/S5.10/S5.5 pins.

## S5.13 (queued 2026-09-24 09:40, Morten): mono spectrum analyzer should MIRROR on the right side
Current mono mode (S5.7) draws ONE mix trace for SID (TrackerSpectrumAnalyzer, tracker + jukebox). Morten wants the mirrored look: render the single mono trace mirrored on the right side (left channel normal, right channel mirrored — visual symmetry like classic mono analyzers). Scope: TrackerSpectrumAnalyzer mono path only (per-source discrimination stays: SID = mirrored mono, others = L/R pair); test asserts the mirrored rendering for SID and L/R for others; desktop only visible surface (phone hides the analyzer). UI-only, no Rust. Spawn after S5.12 (chip fixes) lands — or as parallel UI batch if S5.12 runs long (different files: .vue component vs Rust, zero conflict risk — prefer parallel for responsiveness, Morten is testing visuals on PC).
