...
| 6 | **Clipping headroom** (M5). Either (a) do nothing, or (b) for hi-fi, soft-clip the mix instead of hard-clamping `i16` at `engine.rs:1465`. **DECIDED 2026-09-22: accept + document** (a); evidence M5/M6 (469 clips hi-fi on, all isolated single samples, max overshoot ~×1.19). Fix path if ever audible: unclamped f32 hi-fi output, not soft-clip/taper. | (a) None expected: isolated single samples. (b) None expected. | 0 / ~1 compare per sample | Low | S | (b) moves the hi-fi hashes only for songs that clip; none of the 5 baseline songs clip in their first 10 s |
