/**
 * A tracker sampler voice over plain Web Audio.
 *
 * One instrument, one sample, N monophonic voices -- which is what a tracker
 * instrument is. It uses `AudioBufferSourceNode` rather than an AudioWorklet:
 * a module player needs sample playback with volume, panning and pitch, and
 * nothing an AudioWorklet would buy is worth its per-node cost at 32 channels.
 *
 * No WASM, no worklet, no synth engine. The only Web Audio nodes it creates
 * are `createBufferSource`, `createGain` and `createStereoPanner`, so it runs
 * anywhere an `AudioContext` does.
 *
 * It is fed a `TrackerSamplerConfig` plus PCM rather than any host's patch
 * format -- see `loadSample` for the library path, and this app's
 * `src/audio/mod-instrument.ts` for a host that decodes its own.
 */
...
export class TrackerSamplerInstrument {
  ...
  /**
   * Load a `TrackerSample` straight from an importer.
   *
   * The library path. A host with its own instrument format decodes it and
   * calls `load` instead.
   */
  ...
  /**
   * Build the playable buffer from decoded PCM and a config.
   *
   * `data` is interleaved if `channels > 1`, though tracker samples are mono
   * and the conditioning below only runs for the mono case.
   */
  async load(
  ...
  private conditionSample(
    ...
    if (state && state.loopMode === 'forward') {
  ...
  noteOn(
    ...
    if (!this.audioBuffer || !this.samplerState) {
      console.warn(
        '[TrackerSampler] noteOn skipped - buffer or state not ready',
      );
      return;
    }
  ...
  destroy(): void {
    console.log(
      '[TrackerSampler] Destroying instrument, active voices:',
    ...
    console.log('[TrackerSampler] Destroyed successfully');
  }
  ...
  private resolveVoiceForTrack(trackIndex: number | undefined): number {
    ...
    warnOccasionally(
      ...
      () =>
        `[TrackerSampler] No free voice for track ${trackIndex} (limit ${this.voiceCount}); reusing`,
    );
  ...
  noteOnAtTime(
    ...
    if (!this.audioBuffer || !this.samplerState) {
      console.warn(
        '[TrackerSampler] noteOnAtTime skipped - buffer or state not ready',
      );
      return undefined;
    }
      ...
      source.onended = () => {
        if (this.activeVoices.get(voiceIndex)?.source === source) {
          ...
          if (modInstrumentDebug()) {
            console.warn(
              `[TrackerSampler] Voice ${voiceIndex} auto-freed (one-shot ended), activeVoices now=${this.activeVoices.size}`,
            );
          }
        }
      };
    ...
    if (modInstrumentDebug()) {
      console.warn(
        `[TrackerSampler] noteOnAtTime allocated voice ${voiceIndex} for note ${noteNumber}, loop=${source.loop}, activeVoices=${this.activeVoices.size}`,
      );
    }
  ...
  setVoiceGainAtTime(
    ...
    if (!voice) {
      warnOccasionally(
        ...
        () =>
          `[TrackerSampler] setVoiceGainAtTime: voice not found: ${voiceIndex} active voices: ${Array.from(this.activeVoices.keys()).join(',')}`,
      );
      return;
    }
