...
| Date | Phase | Change |
|---|---|---|
| 2026-09-20 | feat | **Song Edit B2b: `.cmod` v5 carries the AHX file — an editable AHX song saves, reloads and exports from its own bytes (D133).** `TrackerSongFile` v5 adds `data.ahxFile` (base64 of `buildAhxFile(doc, slots, title)`), written by `serializeSong` only when `moduleFormat === 'ahx'`, the store holds a doc, and `currentAhxBytes()` produces one; an HVL song (`moduleFormat: 'ahx'`, doc `null`) never embeds, and `handleSaveSongFile` refuses it with the D131 text plus a belt that refuses any AHX serialization that came out without a file. The load version gate became a range (1..`CURRENT`=5), so every v4 file (native, MOD, XM, S3M, AHX) still loads; v1–v5 gate refusals are pinned (0, 6, 99, -1, 4.5, `'5'`, `null`, NaN). On load a valid `ahxFile` beats any source record: doc, grid and instrument slots are rebuilt from the bytes and the saved row model is ignored. A bad `ahxFile` (not text, not base64, `length % 4`, over the 512 KiB cap — refused before `atob` — or not an AHX file) is a `console.warn` + read-only display, never a throw. Pre-v5 `.cmod` files keep the no-source export refusal (pinned both in-memory and saved-then-loaded). **B2a re-review non-blockings folded in:** `transposeTrack` got the flush-before-`pushHistory` the other bulk ops have (N1, with test + negative control), the refusal-beats-warning branch is tested with a revert-red control (N2), native load-then-undo pinned (N3), the Jukebox put-back re-parse behaviour characterized (N4), the stale exporter test title fixed and the Play/Export-heals-byte-less-song path tested end to end (N6). **Exporter (merged with D131/D132):** `ahxExporter` prefers the embedded file — it is rebuilt from the live document at snapshot time, so it is never stale — and exports it byte for byte; a song without one falls back to the record overlay or the HVL→AHX conversion exactly as D132 shaped it. **Review (dawn-willow, read-only): PASS**, 8 non-blockings logged for B3: the 512 KiB cap is enforced on read only (a very long title or instrument name can write a file load then refuses), an oversized instrument set makes save refuse with the wrong ("plays from the original file") text, unedited byte-exactness is only corpus-checked at doc level (karma.ahx end to end), per-slot mixer `volume` is dropped when slots are rebuilt from the file, `CURRENT_SONG_FILE_VERSION=4` still lives in the library beside the store's 5, `AhxInstrumentPage` blames "saved before .cmod files could carry AHX songs" for a rejected v5 file, this entry (now written), and `projectAhxPatterns` sits outside `adoptAhxDoc`'s try. Branch suite 3207/200; merged main (with D131/D132 + kbd pin) gates: lint 0, vue-tsc 0, tests green; negative controls for the v5 round-trip and the N1 flush red-then-restored in `.ai/checks-songedit-b2b.txt`.
| 2026-09-20 | feat | **Song Edit B1: `AhxDoc`, ops, size budget, projection and `buildAhxFile` — the model an editable AHX song is built on (D130).** First batch of Song Edit (`.ai/plan-ahx-editing.md`); **nothing in the app imports it yet**. `src/audio/tracker/ahx-doc/` (pure TS, the one Vue import is `markRaw`): `AhxDoc` is an immutable, `markRaw`-ed, top-level-frozen structure (header, `positions[]` of 4 tracks + transposes, the file's own `tracks[]` with shared and unreferenced tracks kept, raw untrimmed song name, `base?: Uint8Array`); instruments stay in the slots and `buildAhxFile({ doc, slots, title })` joins them through the D128 writer (`{ bytes, titleAltered, instrumentNamesAltered }`; the exporter's `songNameFor` title rule, copied verbatim so an unedited song stays byte-identical). Ops are pure `(doc, ...) -> { ok, doc, ... } | { ok: false, reason }`, structurally shared, a no-op returns the *same* doc: `setStep`, `setTrack`, `allocTrack` (reuse the first unreferenced blank track >= 1, else append, `append: true` forces a new one, refuse at 256), `assignTrack`, `setTranspose`, `makeUnique`, `insertPosition` (blank / duplicate / duplicateUnique), `deletePosition` (refuses the last), `movePosition` (all three return `mapPosition`, and `restart` and the subsong starts follow it; a deleted one falls to the position now at its index), `setSpeedMultiplier`, `setRestart`, `setTrackLength`, `setSongName`; every op that grows the file is refused when it would pass the 16-bit `nameOffset` (`Song is 65,344 of 65,535 bytes; a new track needs 192.`) and a change that does not grow it is never refused. `ahxUsedBytes`/`ahxSizeBudget` equal the serialized `nameOffset` (compared on real songs and on every new-song option). `entriesToTrack` is the inverse of the row projection and refuses, naming the row, what a step cannot hold (volume column, second effect, frequency, note outside 1..63, instrument > 63, macro letter past `F`, row >= `trackLength`, a row twice); `projectAhxPatterns` is the library builder with the three new options. **Library** (`packages/tracker-playback`, `import/ahx-patterns.ts`, defaults unchanged so HVL and `ahx-patterns.test.ts` are untouched): `buildAhxTrackerPatterns(song, { latchInstruments, stableIds, clampNotes })`, `ahxNoteFromTrackerText` (1..63; text past 60 continues the name scheme: 61 `C-6`, 62 `C#6`, 63 `D-6`, pinned by a fixture) and `ahxPositionPatternId` (`ahx-pos-<n>`). **App:** `defaultAhxInstrument()` in `ahx-instrument-edit.ts` (volume 64, wave length 3, envelope 1f->64 / 10f->48 / s0 / 10f->0, PList speed 1 with one sawtooth row). **Measured:** `buildAhxFile(docFromBytes(x)) == x` on **62/62** `.ahx` demos (also a fixed point), and every cell of every position of all 62 projects and reads back to its track; the default new song is **273 bytes** (arithmetic confirmed by the real writer) and renders through the real wasm at peak 0.348 at every pitch tried (blank song exactly 0); all 6 track lengths x 4 speeds x 4 names are exactly `14 + 8 + 3*trackLength + 26 + names`. **Plan correction:** the plan says a non-blank track 0 is "never in the corpus" (section 1.4) — **36 of the 62** demos store an explicit, non-blank track 0 (flag clear) that is an ordinary track there; the ops treat a *blank* track 0 as copy-on-write (they refuse to write into it) and a non-blank one as any other, and a blank position inserted into such a song needs four blank tracks of its own, so N4's byte-identity holds for the 26 songs with a blank track 0 and is resolved-playback identity for the 36. Also measured: 29/62 have an unreferenced track (counting track 0), 61/62 a shared one, 42/62 non-zero transposes. **Bug found by the unit tests, not the property test:** `allocTrack` returned the previous last index for an appended track (the property test never checks the returned number). Tests: `ahx-doc-corpus` (20: T1 A + the N1-N7/A1-A3 matrix in three tiers — bytes, resolved playback, and real-engine PCM on a 12-song subset), `ahx-doc-ops` (30: every op at its boundaries, exhaustive small-song position maps, size limit, 300 seeded random op sequences with size pressure), `ahx-new-song` (14), `ahx-doc-entries` (13, incl. the 61..63 clamp fixture); helpers `helpers/ahx-doc-fixtures.ts`, `helpers/ahx-render.ts`. A1/A2 pick the first note the engine reaches in 8 s on a pitched instrument (no fixed-note or noise PList row); 5 songs have none and are skipped, pinned by name. Not in this batch: T1(A1)-through-the-row-model+store and T1(C) (store, B2a/B2b), the store, the UI. Negative controls (latch on, restart not remapped, budget off by one, track-0 guard dropped, writer without `base`) all RED then restored, in `.ai/checks-songedit-b1.txt`. |
| 2026-09-20 | fix | **Export text stripped to plain truthful lines; HVL songs export `.hvl`; an HVL song exports `.ahx` only when it fits 4 tracks (D131).** Morten, 2026-09-20 17:41, verbatim: "the export text for ahx makes no sense at all.. it should export a new song made from scratch as well.. we need to remove all that garbage text description, and allow hvl export.to ahx as well if song had more than 4 tracks." **Text:** every user-visible export string was audited against `serialize`. The AHX row is now one sentence ("Saves the song as an .ahx file, with your instrument and title changes."); the Author/BPM/speed-multiplier lecture is gone, and author/BPM show one line ("Author and BPM changes aren't saved.") only when one of them differs from the import's `Unknown`/125, which is the one case where it is true and the user can act on it (an unedited song, read back through the store, warns nothing: pinned). The MOD/XM/S3M rows lost their filler descriptions (empty description = no line) and say "Not available yet."; refusals are one plain line each ("XM songs can't be saved as AHX.", "This song has no original file to export from.", "AHX songs can't be saved as HVL."); the `.cmod` refusal, the AHX instrument banner tooltip and the toolbar tooltip no longer claim "AHX songs only" or that edits die with the session. **HVL row** (`hvl-exporter.ts`, label "HVL (Hively Tracker)", `.hvl`): `serializeAhx(parse(source), { base })`, byte-identical to the source on all 7 demo `.hvl` files through the real store path. Only the title is overlaid (HVL stores a song name in the same string table; an unchanged title keeps the file's name verbatim, same rule as AHX). **No instrument overlay, on purpose:** `importAhxToTrackerSong` gives an HVL song *no* instrument slots (it would need its own `instrumentFormat`) and `updateAhxInstrument` rejects an edit without a slot, so the store cannot carry an HVL instrument edit; pinned by a test, nothing invented. `base` is not load-bearing for HVL (its two gaps, the blank-first-track flag and the inert instrument bits, only occur in AHX files): dropping it stays green, kept as the writer's contract. **HVL -> AHX** (`hvl-to-ahx.ts`, `convertHvlToAhx` on the parsed model, then `serializeAhx` with no base): enabled only when every one of these holds, else one refusal line pointing at HVL: data only on channels 0..3 in every position (a higher channel holding only blank tracks is dropped; data on a higher channel is refused, not moved down, because a move changes its stereo position); no second effect column (the writer would refuse it); no note or instrument above 63; every instrument passes `ahxInstrumentProblem(.., 'ahx')` (AHX PList codes commands 0..5, 12, 15; HVL has all 16); no `EF1` in a version-1 HVL song (`stepfx_3` reads it only when `version >= 1` and an AHX file is always played as version 0). The HVL header's mix gain and default stereo have no AHX field, so an enabled conversion says so ("AHX files don't store this song's stereo and volume mix."). Morten's case: "AHX files have 4 tracks; this song uses 8. Export it as HVL instead." **Measured: none of the 7 demo `.hvl` files fits** (channels used 6, 11, 7, 6, 7, 6, 6; `moderate_sellotaping` declares 8 and uses 7; all 7 also use the second effect column), so the positive path is tested on models derived in memory from a real parsed file (cut down to 4 channels, second column cleared), never as a fake fixture; each rule is then broken on its own. **Not done, noted for the B5 gate:** exporting a song made from scratch. It needs no source bytes: it rides the Song Edit doc-based serialization (the song document, not the imported file). Until then such a song is refused with "Songs made from scratch can't be exported yet." Tests: `song-export-hvl` (32, new), `song-export-registry` (27), `song-export-dialog` (24), `ahx-exporter-store` (21), `ahx-exporter-corpus` (4: the 7 `.hvl` files now export identically and refuse as AHX). Negative controls in `.ai/checks-export-ux.txt`: flipping `restart` in the HVL base handling (11 red), ignoring the title overlay (3 red), a `channelsWithData` that sees nothing (12 red); dropping `base` stays green (see above). Suite 3044 green (194 files); lint 0; vue-tsc at its 11-error `#q-app` baseline. |
| 2026-09-20 | fix | **Review round on D131: the converted-HVL name, the untested converted path, and a check that could disagree with serialize (D132).** **Bug 1:** `ahxExporter.serialize` judged the title against the converted song (`format: 'ahx'`), whose import fallback is "Imported AHX", while the store's title for an unnamed HVL is "Imported HVL"; an unedited unnamed HVL therefore wrote "Imported HVL" into the `.ahx` as its name (the c23e237 class). `withStoreEdits` now takes the name's source song separately and `serialize` passes `planned.base`, the same song `warnings()` reads; an unedited song writes the file's own (empty) name. **Bug 2:** nothing ran `ahxExporter` on a fitting HVL `TrackerSongFile`. `src/tests/helpers/fitting-hvl.ts` cuts a real demo `.hvl` down in memory to channels 0..3 and writes it as `.hvl` bytes; the tests import it through the store path and pin `converted: true`, no instrument overlay, the writer run with no `base`, `HVL_MIX_NOTE` and warning order, the empty-name case, and the dialog's enabled AHX row and download. **Check vs serialize:** `convertHvlToAhx` only read tracks a kept channel plays, but the writer writes every track, so a second effect column or a note above 63 in an unplayed track passed `check` and threw at download. The converter now runs the writer once on the converted song and refuses in the writer's own words ("AHX files can't hold this song: track N row R has a second effect column, which AHX has no room for. Export it as HVL instead."), which also covers the 16-bit table-offset limit. **Wording:** the channel refusal said "this song uses N" where N is the highest track with data; it now says "this song reaches track N". **Correction to D131:** `base` *is* load-bearing for an HVL file whose instruments carry the inert bytes 9..11 or the top two bits of byte 19 (no demo does); a control that sets them in a demo's bytes now goes red when the HVL exporter drops `base`. **Scope note:** "plays the same" is a claim about this app's engine, not about third-party AHX players. |
| 2026-09-20 | feat | **Song export B2: exporter overlay, registry and a format-agnostic Export dialog on the tracker page (D129).** Second batch of Song Export (`.ai/plan.md`), on top of the D128 writer. `src/audio/tracker/song-export/`: `ahx-exporter.ts` is an *overlay*, not a serialization of the store (the store's row model of an AHX song is display only): the file's structure comes from the source bytes `snapshotEditorSong(store)` attaches (`ahx-source`), the instruments from `slot.ahxData` and the name from the title, then `serializeAhx(merged, { base })`. A title that still equals the import's own derivation (`name.trim() || 'Imported AHX'`) keeps the file's name verbatim (two corpus files have edge whitespace); an edited title is written with NUL removed and characters above U+00FF replaced by `?`, and `warnings()` says so only then. `AhxEncodeError` is wrapped as `SongExportError`; a missing/invalid slot is a `SongExportError`, never a partial file. No source record (loaded from a `.cmod`, or an edit that was only 'kept'), an HVL song, or another module format make the exporter *unavailable* with the truthful reason as visible text. `registry.ts` holds the four rows (ahx, mod, xm, s3m; the last three are placeholders, `available: false`, reason 'Writer not implemented yet') and `describeSongExporter`, the one place a row's state is decided; `file-name.ts` (the `.cmod` character policy, underscores collapsed, 64 chars, `song` fallback) and `download.ts` (Blob + attached `a[download]`, URL revoked on a timer). `SongExportDialog.vue` reads only the registry (native controls, `data-testid` on every button, Esc/backdrop/Close, Tab trap, focus returns to the opener) and asks `getSong` again at every download, so a song change between open and click cannot export stale state; `TrackerPage.vue` gained an `Export…` button after Save in both toolbars plus the dialog (~25 lines). The `.cmod`-refusal message now points at Export. **Trap found by the dialog test:** the dialog must hold the song in a `shallowRef`; a deep `ref` wraps it in a reactive Proxy and `ahx-source`'s `WeakMap` (keyed by the raw object) then finds no source, so the AHX row would have said 'no source file' for every song. **Measured:** unedited export from the song's own record is byte-identical to the source on all 62 `.ahx` demos through the real store path (`ahx-exporter-corpus.test.ts`); the 7 `.hvl` demos are byte-identical at library level only (D128) and unavailable in the UI. A same-length instrument edit changes only that instrument's bytes; a PList row added shifts what follows by 4 bytes and moves `nameOffset`; author/bpm changes change no byte. Tests: `ahx-exporter-corpus` (3), `ahx-exporter-store` (15), `song-export-registry` (25), `song-export-dialog` (21). E2E (headless Firefox, real app): 34 new checks incl. the Blob and the file Firefox wrote to disk byte-identical to `karma.ahx`, an edited export differing only inside instrument 1 and re-loading + playing, HVL/MOD rows disabled with their reasons, the phone-width button, and export after a jukebox handover; the 43-check editor baseline stays 43/43. Negative controls in `.ai/checks-export1.txt`. |
| 2026-09-20 | feat | **AHX/HVL writer: `serializeAhx(song, { base })` is `parseAhx` in reverse, byte-identical on the whole demo corpus (D128).** First batch (B1) of Song Export (`.ai/plan.md`): the serializer only, no registry/UI yet. `src/audio/tracker/song-export/ahx-writer.ts` (app side; `packages/tracker-playback` untouched) writes AHX (`THX`) and HVL (`HVL`) from an `AhxSong`, reusing the library's `serializeAhxInstrument` per instrument and carrying its own small layout walker over `base` (the source bytes) because the parser exposes no offsets. Nothing is clamped: anything the format cannot hold (positions > 1000, trackLength/instruments > 64, model/`trackNr` disagreement, a second effect column in AHX, PList command AHX cannot code, HVL non-blank step with note 0x3f which collides with the 1-byte blank escape, names with NUL or chars above U+00FF, string table past the 16-bit `nameOffset`) throws `AhxEncodeError` and returns nothing. **What the model cannot carry** (two gaps, both filled from `base` only): (1) the blank-first-track flag (header byte 6 bit 7) — the parse synthesizes an empty track 0 and forgets whether one was stored; five version-0 files (`all_that_she_wants`, `classic_cracktro`, `depressed`, `running`, `winter_dreams`) store an explicit all-zero track 0 with the flag clear, so guessing "all zero => flag set" is 192 bytes short there; (2) the loader-ignored instrument bits (core bytes 9..11 and byte 19 bits 7..6: `aces_high` instrument 8, `get_to_the_chopper` instruments 5/10/15). A set flag over a non-blank track 0 refuses rather than silently dropping rows. **Measured corpus result (69 files = 62 `.ahx` + 7 `.hvl`):** with `base` 69/69 byte-identical (confirmed, not just predicted); from the model alone 62/69, the 7 exceptions being exactly the files above (pinned as an exact-diff table: the 192-byte delta with flag/`nameOffset` patched, and the four byte-19 bytes), each parsing back to the same song; fixed point holds on all 69. **Plan correction:** the plan said HVL never clamps restart; `parseHvlBody` does clamp `restart >= positionNr` to `posn-1` exactly as AHX does (only the subsong-start clamp is AHX-only), pinned in the unit test. The writer itself clamps nothing in either format. Tests: `src/tests/ahx-writer-corpus.test.ts` (4) and `src/tests/ahx-writer.test.ts` (77: header bits, packing edge cases, blank flag, string table, HVL, base walker, 38 refusals, seeded read-back property over 300 random songs). Negative controls (bit flip in packing; dropped base flag read => exactly the 5 v0 files) in `.ai/checks-export1-b1.txt`. |
| 2026-09-10 | fix | **A bare sample number reloading the channel volume states it as a step, not a swell — even when a volume slide shares the row (D127).** `butterfly_syndrome.mod` order 9 channel 4 rows 48–63, reported by Morten by ear: instrument 03 should "pump" three times, the engine gave two. The pump idiom: row 48 triggers sample 3 under `C00` (volume forced to 0 — Paula keeps DMA'ing the sample silently), rows 49–53 are empty, then row 54 carries a *bare sample number* (`03`, no note) plus `A08`. ProTracker `mt_PlayVoice`'s sample-number block (`move.b 3(a0,d0.w),n_volume(a6)` then `move.w d0,8(a5)` — the AUDxVOL register) loads sample 3's header volume, 64, and writes Paula's volume register on tick 0, *instantaneously*; `mt_VolumeSlide` then slides `-8` per tick on ticks 1–3 (speed 4), so the channel holds 64 → 40 → 16 → 0 at successive row starts. That tick-0 jump to 64 on an already-sounding voice is the third pump. The engine computed the value but `emitTick0VolumeSlide` emitted it as an *unqualified* volume command, which `SongBank.setVoiceVolumeAtTime` turns into `linearRampToValueAtTime` running from the previous automation event — and with no volume event on the channel since row 48, the ramp glided 0 → 64 across the whole six-row gap. The hard attack became a slow swell, inaudible as a distinct pump. This is the same defect `tracker-set-volume-is-instant` (D-series, jaguar_xj220) fixed for a bare `Cxx`, left unfixed for the case where a slide shares the row because "a volume slide must still ramp". Fix: `emitTick0VolumeSlide` takes `volumeStatedThisRow` (`newVelocity !== undefined` — a sample-number reload, a `Cxx`, an XM/S3M volume-column set-volume) and states the tick-0 level as `'step'` when set; the per-tick slide still ramps as before. Applies to all three volume-slide tick-0 paths (`volSlide`, `tonePortaVol`, `vibratoVol`) and every period format — ProTracker, FT2 and ST3 all write the volume register directly on a volume statement. **Blast radius: 55 of 139 corpus event streams change — 21 MOD, 8 XM, 26 S3M — and every one of the 5130 changed events is the same volume command flipping ramp code 0→'step' at an identical time and value; zero note/pitch/pan/macro/timing deltas, zero `songEnd` deltas.** The scale reflects how common the sample-number-plus-`Axy` pump is in tracker music. Tests: `src/tests/butterfly-syndrome-pump.test.ts` drives the real load path (`importModToTrackerSong` → `buildPlaybackSong` → `PlaybackEngine`) through order 9, reconstructs channel 4's Web-Audio gain curve from the scheduled automation and pins it against the ProTracker `mt_VolumeSlide` trajectory: 64 / 40 / 16 / 0 at rows 54–57 (±1/64), and the row-54 attack as an instantaneous jump of >60/64 from the silent tick before it. **Fails on 570e6fd**: the reload arrives as `ramp: 'linear'` and the "jump" is ~1/64 (0.958 swelled value one tick before row 54). 55 event-stream goldens regenerated. tracker-playback `tsc` and `npm run lint` clean; app `vue-tsc` at its pre-existing 10-error baseline (unrelated `#q-app/wrappers` / `quasar.config.ts`), unchanged by this diff; gitleaks clean. |
| 2026-09-08 | fix | **A stopped S3M vibrato snaps back to the note on the next effectless row (D126).** turbulence.s3m order 37 (pattern 24) channel 6, reported by Morten as "this note ends on wrong pitch after a pitch slide, row 27 → row 31". That channel retriggers G-6 under `E50` (row 27), tone-portamentos back down to G-4 under `G50` (row 28), vibratos `H85`/`H00` (rows 29–31), then holds the note through *empty* rows 32–43 before the vibrato returns on row 44. The engine froze the last vibrato deviation for the ~1.45 s the note rang — ~812.73 Hz where G-4 is 784.93 Hz, **+60.3 cents sharp** against channels 0/2/3 for twelve rows. st3play `digcmd.c` `docmd1` (tick 0), the `ch->cmd == 0` arm: `// fix speed if tone port noncomplete` / `if (ch->aspd != ch->aorgspd) { ch->aspd = ch->aorgspd; setspd(ch); }`. `s_vibrato` only ever writes `ch->aspd = ch->aorgspd + offset` and never touches `ch->aorgspd` (the tone-portamento and E/F slides *do* keep it in step), so the first row carrying no vibrato command snaps the channel period back to the note. This is the ST3 exception to D74/D75 — FT2 and ProTracker-MOD hold the offset across effectless rows (jt_911.xm depends on it), and the engine's `continuesVibrato` path is left doing that for them. New `FormatProfile.pitchResetsAfterEffectlessRow`, true for `S3M_PROFILE`/`S3M_AMIGA_PROFILE` only. Two touch points: `processEffectTick0` stops treating a non-vibrato row as a vibrato continuation for those profiles (the existing "emit at least one pitch" fallback then states `currentFrequency`, the un-modulated base), and the engine's trailing per-row pass — which already re-runs unfinished tone portamentos on the tracks a row omits — emits one base-pitch command for a track whose vibrato has stopped, mirroring the `if (ch->aspd != ch->aorgspd)` guard so a later effectless row is a no-op. **Blast radius: of the 139 corpus event streams, 8 change, all S3M, all pitch-only** (zero note / volume / macro / timing deltas) — energia (a vibrato'd pad fading out over 20 empty rows, 41 cents flat throughout), funtro (8), global_motion_vol_1 (35), satellite_one (20), seaside_hotel (19), starshine, the_probe_maintheme, turbulence; every delta moves the held pitch onto the note it should rest on. MOD and XM streams byte-identical. Tests: `src/tests/turbulence-s3m-pitch-slide.test.ts` drives the real load path (importer → tracker store → `TrackerSongBank` → `buildPlaybackSong` → `PlaybackEngine`) to order 37 and pins the held channel-6 pitch within 5 cents of the tone-portamento target — **fails on 15922d5 at +60.3 cents**, passes after. Eight S3M event-stream goldens regenerated. Suite 2003 green; eslint and vue-tsc clean. |
| 2026-09-06 | fix | **S3M volume slides read ST3's nibbles, not ProTracker's (D125).** Reported as "some .s3m songs still don't sound right" while MOD and XM were clean. `Dxy` (and the `Kxy`/`Lxy` that share ST3's routine) was resolved through the MOD/XM reading — mask to one nibble, up wins — but OpenMPT's `CSoundFile::VolumeSlide` (Snd_fx.cpp, fetched 2026-09-06) applies that masking *only* to `MOD_TYPE_MOD | MOD_TYPE_XM | ...` and runs a second reading for everything else. Three differences, all audible: **(1) a 0xF nibble makes the slide fine** — `DxF` is one step up on tick 0, `DFy` one step down — where the up-nibble-wins reading turned `DFy` into a slide *up* by 15/64 on every tick, slamming the channel to full scale inside one row; **2500 cells in the 40-module demo corpus are written that way, and two modules are made of almost nothing else** (`return_to_saturn.s3m`, 1449 of 1474 `D` commands; `o-79642.s3m`, 557 of 557). **(2) The down nibble wins** when both are set (`D48` slides down by 8, not up by 4) — 284 corpus cells. **(3) `D0F`/`DF0` slide on every tick including the first**, because OpenMPT's fine branch declines them and the general block then runs them from tick 0. Separately, **`fastVolumeSlides` was parsed and logged but never reached the engine**: the header property `Load_s3m.cpp` reads as `SONG_FASTVOLSLIDES` — `cwtv == trkST3_00` (0x1300 exactly, *not* "older than 3.20") or header flag 0x40 — makes ordinary slides step on tick 0 too, and seven corpus modules qualify (five by version, two by flag). **Shape:** two new `FormatProfile` fields, `volumeSlideNibbles: 'modxm' | 's3m'` and `fastVolumeSlides`, both stated explicitly on every profile; `resolveVolumeSlideDelta` becomes `resolveVolumeSlide`, returning `{delta, mode, firstTick}` so the fine/normal split is decided where the nibbles are read rather than at three call sites; the three tick-0 call sites collapse into `emitTick0VolumeSlide`. `fastVolumeSlides` is per-file, so it rides the D59 chain (importer → song file → store → `buildPlaybackSong` → `profileForFormat`) and *composes* with `amigaLimits` rather than doubling the profile table — `profileForFormat` spreads the one boolean onto whichever S3M constant the pitch model chose. **Blast radius measured:** of the 139 corpus event streams exactly 13 changed, all S3M; every MOD and XM stream is byte-identical. Tests: `src/tests/s3m-volume-slide.test.ts` (14; 12 confirmed failing against the old code) — the three nibble readings at the engine, the fast-slide flag from both header routes, the version *equality* pinned against the tempting `< 0x1320` reading, and MOD/XM/native pinned inert. Suite 1951 green; eslint and vue-tsc clean. **Still open, found in the same audit and deliberately not fixed here:** ST3's single shared per-channel effect memory (OpenMPT `kST3EffectMemory` / `UpdateS3MEffectMemory`, where any non-zero parameter becomes the memory for volume slide, porta up/down, tremor, arpeggio, retrigger, tremolo and Sxy alike) — 1511 corpus cells read a different parameter under it than under our per-command memories, concentrated in `energia.s3m` (28% of its zero-parameter cells) and `point_of_departure.s3m` (9%). It needs the `isST3` header fingerprint OpenMPT gates it on (`nonCompatTracker`), which the parser does not yet compute, so it is a task of its own — written up with the counts, the quoted `UpdateS3MEffectMemory`/`nonCompatTracker` sources and a shape for the work in `NOTES-s3m-effect-memory.md`. |
| 2026-09-05 | fix | **MODs written outside ProTracker's three octaves get the seven-octave period range (D124).** Reported on `DOPE.MOD` as "samples sound out of tune, amplitude off sometimes". The module is a 28-channel `28CH` file whose periods run 3424..28 — the first and last entries of OpenMPT's `ProTrackerPeriodTable[7*12]`, which extends the classic 36-entry table by two octaves either side. The engine clamped every note through `createAmigaPitchModel`'s 113..856, so **3137 of the module's 6589 notes (48%) played at period 113 and 29 more at 856** — up to two octaves off, and the notes' own MIDI numbers (which the importer derives unclamped from the same period) said so. Import was never the problem: `periodToFrequency` writes the literal period's frequency, and the golden stream showed `midi 14` next to `32.67 Hz` (= period 856), the two disagreeing by an octave. **Fix follows OpenMPT's own split** (`Load_mod.cpp` `ReadMOD`, fetched 2026-09-05): every MOD loads with `m_nMinPeriod = 14 * 4; m_nMaxPeriod = 3424 * 4;` and narrows to the Amiga range only when `onlyAmigaNotes && !hasRepLen0 && (M.K.|M!K!|PATT)` — reproduced verbatim as `modUsesAmigaLimits` in `mod-parser.ts`, exposed as `ModSong.amigaLimits`, and threaded through the *existing* `amigaLimits` flag (importer → store → `buildPlaybackSong` → `profileForFormat`) that S3M already used. New `PROTRACKER_EXTENDED_PROFILE` differs from `PROTRACKER_PROFILE` in the pitch model alone; `createAmigaPitchModel({ amigaLimits: false })` swaps both the clamp (14..3424) *and* the table arpeggio and glissando step through — a period the clamp allows but the table cannot name would snap a glissando straight back into the old three octaves. **Polarity is per format and deliberate:** for S3M only an explicit `true` narrows (ST3's default is wide), for MOD only an explicit `false` widens (ProTracker's default is narrow), so a hand-authored song and every pre-existing save keep the behaviour they had. Note this is a note-range test, not a channel-count test: the 4-channel `purple_motion_-_sundance.mod` gets the wide range for its single out-of-range note (OpenMPT's "fixes mod.mothergoose"), and the 8-channel `starlitdeception.mod` is unaffected because it never leaves the range. **Blast radius measured, not assumed:** of the 139 corpus event streams, exactly one — `DOPE.MOD` — changed; the twelve signature-less Soundtracker files that also fall to the wide range are byte-identical. Tests: `src/tests/mod-extended-period-range.test.ts` (11: clamp, glissando snapping, cross-octave arpeggio, profile selection, the parser's verdict on four real modules, and the flag surviving import) and `src/tests/dope-mod-tuning.test.ts` (2: the real module driven through a real engine from order 15, asserting every scheduled note lands within a quarter-tone of the MIDI number its row names — 215 notes failed that in a 20 s window before the fix, 0 after). Suite 1912 green; eslint and vue-tsc clean. |
| 2026-09-05 | fix | **`setPatternLength` now survives loop wraps: the engine's `patternsById` map is rebuilt for the edited pattern (D123).** Live-editing a pattern's row count mid-song honored the edit only for the current pass: at the next pattern boundary — including the sequence wrap back to position 0 — `loadPattern()` re-read the pattern through the engine's `patternsById` cache, which `setPatternLength` never updated. The map still held the PRE-edit pattern object, so the wrap reloaded the old row count and every pass after the first played the un-edited length. The hole predates the P4 pattern-index cache but is correct under it: the WeakMap step index keys on pattern-object identity, and with `patternsById` now pointing at the replacement object, `loadPattern` indexes the new object naturally (the old object's cache entry becomes unreachable and GC-eligible). Fix is one behavioral line — `setPatternLength` sets `patternsById` to the replacement object alongside the existing `song.patterns` swap — plus doc-comment corrections on `patternsById` (its "cannot go stale" claim was false for values since the P4 split) and the step-index cache note. Audited for sibling holes: `setPatternLength` is the only engine path that replaces a pattern object outside `loadSong`. Regression test `packages/tracker-playback/src/__tests__/engine-pattern-length-loop-wrap.spec.ts`: two-pattern looping song, shorten p0 4→2 rows mid-pass, drive scheduling past the wrap — the second pass of p0 must schedule `[0,1,0,1,...]`, not the pre-edit `[0,1,2,3]`; proven FAILING on base 3b98ef3 (second pass played `[2,3]` of the stale 4-row geometry, evidence in `.ai/checks-regression-prefix.txt` on the worktree) and green after the fix. |
| 2026-09-05 | fix | **Songs load while the AudioContext is suspended; only playback waits for it (deep-link fix cycle 2, D122).** Opening a `?demo=` deep link on a fresh browser tab never loaded the song: the load path was gated on the AudioContext reaching `running`, which requires a user gesture the fresh tab hasn't had. Three hard gates plus one unbounded await conspired: (a) `applySongFile` **awaited `audioContext.resume()` unconditionally** — without a gesture Chrome leaves that promise pending forever, so the load hung before touching a single instrument; (b) `syncSlots` deferred all instrument construction whenever the context wasn't running (it too awaited `ensureAudioContextRunning`, whose first `await ctx.resume()` could hang the same way, and otherwise polled up to 10 s before giving up and returning); (c) `ensureInstrumentInternal` skipped instrument construction outright when the context wasn't `running`. **Fix:** the song now loads fully while the context is suspended — (a) `applySongFile` makes only a bounded best-effort resume attempt (`Promise.race` against 300 ms) and proceeds either way, keeping the "click anywhere to enable audio" warning; (b) `ensureAudioContextRunning` gained a `maxWaitMs` parameter (default 10 s preserved for the playback paths, which still gate on a running context exactly as before) and now bounds **every** `resume()` await — a pending-without-gesture promise can no longer stall anything; (c) `syncSlots` drops its defer-and-return: a bounded 500 ms resume attempt, then instruments are built regardless of state, with `wasSuspended`/`needsAudioContextResume` armed; (d) `ensureInstrumentInternal` builds on `suspended` too (only `closed` still blocks construction). Sound-identity after resume is preserved by the pre-existing recovery net, now actually reachable: `needsAudioContextResume` keeps the `onstatechange` handler armed, so on the first user gesture the bank rebuilds all suspended-born instruments via `syncSlots` (the dispose-and-rebuild path that previously only ran for deferred syncs), and note events scheduled before the gesture stay queued in `ScheduledEventQueue` and flush on resume. `ensureActiveInstrument` (pure UI state), post-fx `onSongLoad`, `decodeAudioData` (asset-encoder path only) and the play-button/jukebox gesture gate were audited and needed no change. New test `src/tests/deep-link-suspended-load.test.ts`: a suspended context whose `resume()` returns a never-settling promise (mirroring real fresh-tab behaviour) — `applySongFile` must complete with the song loaded and `syncSlots` must build instruments instead of deferring; both proven FAILING on main (each times out hung on the pending `resume()` promise, exactly the reported symptom) and passing after the fix. Suite 1712 green; vue-tsc/eslint/quasar build clean; gitleaks clean. |
| 2026-09-05 | fix | **Deep-link fix cycle 1 (review M1/M2/S1, D121).** The review round on D120 found the copied link dead on arrival: the path-style URL `‹origin›/synth/tracker?demo=…` (a) is no served document in a hash-mode static deployment (`vueRouterMode: 'hash'`, dist rsynced with no SPA fallback — 404 before any code runs), and (b) even where a fallback exists, the pathname is not the router base `/synth/`, so `createWebHashHistory`'s initial normalization can discard the `?demo=` search before TrackerPage reads it — the feature's core claim never round-tripped. **M1:** `buildDemoLink` now emits a search-on-base URL — `‹origin›‹BASE_URL›?demo=‹encoded file›` (e.g. `https://host/synth/?demo=amiga%2F12TH.MOD`) — which normalization leaves alone (`#/tracker` is appended into the hash; `window.location.search` keeps the param, where `readDemoLinkParam` already reads). `DEMO_LINK_ROUTE` is gone. **M2:** new integration test `src/tests/demo-deep-link-router.test.ts` drives the production path — a real `createWebHashHistory('/synth/')` router with the '/'→'/tracker' redirect shape, the document seeded with the exact string `buildDemoLink` emits, then an initial navigation — and pins the differentials that matter: the post-navigation pathname must be the router base `/synth/` (the one document a hash-mode static host serves), the demo param must stay readable in `location.search`, and the old path-style format is pinned FAILING on the pathname rule (jsdom cannot reproduce the in-browser search wipe, so the pathname differential is the honest pin here; the exact-string pin on `buildDemoLink`'s output catches any regression back to a path-style emission). **S1:** the hand-rolled context menu in `DemoSongBrowser.vue` is replaced by `<q-menu context-menu auto-close>` anchored on the song button, matching the existing JukeboxPanel/bug-report pattern — q-menu brings long-press on touch (iOS Safari never fires `contextmenu`, so the custom menu made the feature unreachable on iPhone), keyboard open, focus management, aria semantics and scroll dismissal; the ~90 lines of menu state/globals/styles are gone. **Minors:** a manifest *fetch failure* no longer reports "song no longer published" — TrackerPage branches on `useDemoManifest`'s error state and reports the manifest problem, reserving the "no longer in the demo list" warning for a successfully loaded manifest with no match; a deep link skipped because a song is playing now **strips the `?demo=` param** from the URL (`history.replaceState`, route untouched) so the address bar cannot promise a load that did not happen; the clipboard fallback adds `setSelectionRange(0, len)` for iOS Safari and restores the user's prior selection. D120's "first context-menu pattern" claim corrected in place (see above). Tests +5 (14 deep-link tests total); suite 1710 green; tsc 48-error baseline unchanged (none in touched files); vue-tsc/eslint/quasar build clean; gitleaks clean. |
| 2026-09-05 | feat | **Demo deep links: right-click a demo song → "Copy link", and a link that auto-loads that song on open (D120).** The link key is the manifest's relative `file` path — the one identity every surface already agrees on (demo browser keys its buttons by `file`, jukebox entries dedupe/pin/refill by `file`), and it survives manifest regeneration where titles and index positions do not (`public/demos/index.json` is regenerated on deploy). URL format `‹origin›‹BASE_URL›tracker?demo=‹encoded file›` via `src/composables/demo-deep-link.ts` (`buildDemoLink`, `readDemoLinkParam`, `findDemoSongByFile` — pure, tested). Context menu is built into `DemoSongBrowser.vue` (minimal fixed-position menu matching the dialog's panel styling; closes on outside pointerdown, Escape, resize, dialog close), so both surfaces that show the demo list — tracker "load" mode and jukebox "add to playlist" mode — share the same component and both get it for free; the menu reuses the app's existing `<q-menu context-menu>` pattern (as JukeboxPanel's playlist rows and the pattern-editor bug-report right-click already do — an earlier draft of this entry wrongly claimed no prior context-menu pattern existed; corrected 2026-09-05, D121). Copy uses the Clipboard API with the execCommand fallback (`src/composables/clipboard.ts`) and confirms via a Quasar toast ("Link copied — opens ‹title›"; negative toast when both paths fail). Deep-link load: `TrackerPage.onMounted` parses `?demo=`, loads the manifest through the shared `useDemoManifest` (in-flight sharing with the browser/jukebox), looks the file up, and `loadSongFromUrl`s it — the same loader as a manual demo pick, so the hash recording and instrument rebuild paths are identical. Choice: **load but do not auto-start playback** — browsers refuse playback without a user gesture and a silently-stuck autoplay is worse than waiting for the play button; a positive toast says the song loaded. Misses are graceful: unknown/removed song → warning toast "The linked song is no longer in the demo list", no crash; manifest/module fetch failure → negative toast. The load is skipped when a song is already playing/paused (returning from the instrument editor must not clobber the session) and runs un-awaited at the end of onMounted so it cannot hold up keyboard/visualiser setup. The `?demo=` param is left in the URL after load (a refresh re-loads the linked song — honest deep-link semantics). Tests `src/tests/demo-deep-link.test.ts` (10): link shape (origin/base/encoding incl. `&` and spaces), round-trip param read, null on missing/empty param, cross-collection lookup, null on unknown, and clipboard preference/fallback/failure paths. Suite 1705 green; tsc 48-error baseline unchanged (none in touched files); vue-tsc/eslint/quasar build clean; gitleaks clean. |
| 2026-09-04 | feat | **Post-fx rack lands; first stage is the Amiga LED filter, E0x drives it** (D114–D118). The post-fx stack lives INSIDE the replay library per Morten's 2026-09-04 20:22 directive ("make sure the fx stack is inside the new replay library"; plan Revision 3): new `packages/tracker-playback/src/postfx/` (`post-fx-rack.ts`, `post-fx-stage.ts`, `amiga-lpf-stage.ts`, `amiga-filter-math.ts`, `registry.ts`) exported from the package index — the package gains Web Audio *node* code (consistent with its existing AudioContext/AudioNode type surface) while the engine core stays node-agnostic and unchanged. The app keeps exactly three pieces: the Pinia `src/stores/post-fx-store.ts` (library has no Vue/Pinia), the `PostFxFilterControl` component, and `AudioSystem`'s graph wiring — all importing the rack/registry BY PACKAGE NAME (`@another-synth/tracker-playback`), never a relative path into `packages/`. The chain is inserted at the app's only speaker feed: `AudioSystem.destinationNode → PostFxRack → audioContext.destination`, so tracker bank, jukebox (one shared graph through `useTrackerSongHost`) and the patch editor all pass through it; the standalone library sink stays post-fx-free by design. **D114 (topology + numbers):** stage A is a one-pole RC lowpass at fc = 4900 Hz (the A500's fixed filter, per libopenmpt `Paula.cpp` `MakeRCLowpass(sampleRate, 4900)` fetched 2026-09-04), realized PREWARPED (`K = tan(pi*fc/fs)`, b0=b1=K/(1+K), a1=(K−1)/(K+1)) — a documented deviation from the quoted C *as applied*: libopenmpt runs its non-prewarped RC at PAULA_HZ (~3.55 MHz) where fc/fs is tiny, and applying that formula at output rates would land −3 dB ≈ 3.82 kHz @ 44.1 kHz, fs-dependent; ours is hardware-faithful (analog RC does not move with the output rate). Stage B is the LED filter: 2nd-order, fc = 3275 Hz, reproducing `MakeButterworth(fs, 3275, −0.70)`'s ZTransform algebra exactly (that path prewarps already, no deviation). `res = 10^(−res_dB/20) ≈ 1.08393`, so ζ = 0.76645 and Q = 0.65236 are *derived documentation* — params store the libopenmpt-native form `{4900, 3275, −0.7}` and never a bare Q, so a Q/ζ mix-up cannot become a slider default. Golden coefficients (fs 44100/48000/96000, 5 dp) and cascade magnitudes (100 Hz–10 kHz, both states, ±0.1 dB) were derived offline with an independently implemented prewarped bilinear transform and committed as literals; the recursion tests run through the stage's real pipeline (`params → AmigaLpfStage → getCoefficients`), and pins `\|H(3275)\| ≈ −5.25 dB` + monotone response exist specifically to catch a Butterworth-Q (peaks at fc) or ζ-as-Q implementation. Stereo is one stereo `IIRFilterNode` per stage — filters L/R independently with identical coefficients, equivalent to per-channel instances for an LTI stage (no ChannelSplitter ceremony). **D115 (E0x):** MOD E0x = filter ON on even parameter / OFF on odd (`Snd_fx.cpp` `ExtendedMODCommands` case 0x00: `CHN_AMIGAFILTER = !(param & 1)` set on *every* channel — global, and our `scheduledFilterHandler` is likewise global); S3M S00 decodes to the same subtype but ST3.21 dummies it (`s_setfilt, 0`), FT2 dummies E0x, so `FormatProfile.filterToggleCommand` gates dispatch per format (ProTracker true, XM/XM-Amiga/S3M/S3M-Amiga explicit false, native explicit true for hand-typed rows — set explicitly on every profile that spreads PROTRACKER, never inherited). Decode stays decoupled (D94): S00 still decodes, it just never dispatches. D118 (corpus evidence): D93's earlier count already found 36 E0x cells across the 43-module MOD corpus -- cited here as prior evidence for keeping the toggle rather than re-counting (XM E0x and S3M S00 decode but are gated off, so they carry no corpus duty). Closes D93's open gap. **D116 (mode machine):** `post-fx-store` holds OFF/ON/AUTO (default AUTO, persisted via user-settings with the filter params), and the LED means "the LED filter stage is active right now" — in AUTO-off the static RC stays audibly engaged while the LED is dark, which is hardware-faithful (the A500's fixed filter is always in circuit; E0x only toggles the LED stage on top), and manual OFF is the only full bypass. Engine E0x events enter through `tracker-playback-store.createEngine()`'s `scheduledFilterHandler` and are swallowed in manual modes at that one choke point; song load resets AUTO to OFF (hooked at `useTrackerFileIO.applySongFile` AND `TrackerPage.handleNewSong` — the only two song-replacement paths; manual state persists across loads), playback stop cancels queued toggles but keeps the applied state. Scheduled transitions are sample-accurate crossfades between three always-built branches (dry / RC-only / RC+LED) via GainNode AudioParam ramps — no main-thread timers, robust to the engine's up-to-1 s hidden-tab lookahead; mode switches cancel pending toggles so a scheduled E0x cannot fire after a manual override. **D117 (graph wiring):** recorder re-tapped at the rack output so recordings/Export MP3 are what-you-hear; direct-destination re-scan found only song-bank's zero-gain monitor sink (unchanged, benign); visualizers stay pre-rack (scopes show unfiltered content when the filter is on — accepted decision). Params (cutoffs, LED resonance) are user-tweakable in the control's popover with an "Amiga defaults" reset; the control sits in the tracker desktop toolbar + mobile chip strip and the jukebox bar (which already scrolls sideways on phones, so it fits). Tests: package-side pins in `packages/tracker-playback/src/__tests__/` — `amiga-filter-math.spec.ts` (golden coefficients + magnitude pins incl. the monotone/−5.25 dB ζ-vs-Q tripwires), `postfx-stage.spec.ts` (mock-factory seam: topology, crossfade scheduling, cancellation); app-side pins under `src/tests/` — `postfx-store.test.ts` (mode machine, load reset, stop, persistence), `postfx-path.test.ts` (graph topology incl. recorder re-tap), `postfx-control.test.ts` (UI), `e0x-filter-toggle.test.ts` (E00/E01 dispatch through real decode paths; XM/S3M non-dispatch — kept app-side because its songs come through the app's real importers/helpers, so the package test suite would otherwise depend on app code). Suite 1660 green; tsc 48-error baseline unchanged; eslint/vue-tsc/quasar build clean; gitleaks clean. **Fix cycle 1 (review, D119):** the review found the single-slot pending-transition bookkeeping dropped the audibly-applied state when two E0x toggles were queued (stop/mode-switch between them re-asserted the stale pre-t1 state, and resolveLedAt lit early) — both store and stage now carry an ordered pending-transition queue folded in order against the audio clock (stop between t1/t2 keeps t1 applied; mode switch cancels both); immediate transitions crossfade from the effective fader values over RAMP_SECONDS instead of jumping (mode-switch click); setParams fully disconnects the old IIR nodes; derived ζ/Q corrected to 0.76645/0.65236; the change-log table separator/terminator this row had corrupted was restored, and the unescaped pipes inside the magnitude pin above were escaped so this row renders intact (F8). Tests +7 against the pre-fix suite (three stage queue/crossfade pins; three store queue pins; the setParams old-node-disconnection regression pin, which fails on the pre-fix code): two future toggles; stop between t1/t2 keeps t1 applied; mode switch mid-queue cancels both. |
