...
- Tracker scheduling (2025-03): Scheduled tracker note events now queue while the AudioContext is suspended or an instrument is still loading and flush once ready; the near-now “immediate” note path was removed to keep gate pulses/voice tracking consistent. PlaybackEngine lookahead is adaptive (0.5s visible, 1.0s hidden, +0.5s when loops run late) and logs when scheduling approaches its deadline.
- Tracker instrument prep parallelism (2025-03): Slot sync and playback preloads now `Promise.all` instrument preparation, and per-instrument patch load now applies assets and node state in parallel (after the post-load delay), reducing the chance that late worklet prep suppresses the first scheduled notes when starting right after loading a song.
...
- `resolveSamplerGain(sample: ModSample, mod: ModSong)` returns `clamped(sample.volume, 0..64) / 64` for ProTracker/NoiseTracker; header volume `0` now keeps sampler gain at unity so volume slides/Cxx can fade the channel in, while Soundtracker/Unknown still treat missing/0 as unity to avoid silent instruments.
- When the MOD sample header volume is `0` (ProTracker/NoiseTracker), the importer seeds the first note’s volume column with `00` so the channel starts silent but Axx/Cxx can raise it; tone portamento rows (3xx/5xy) skip this seeding and also skip inheriting `lastVolume`, so slides into a new note (even when the instrument column is empty) don’t reset volume back to 0.
...
- Pattern loop state no longer resets on Dxx/Bxx jumps; Bxx overrides Dxx when both appear on the same row (PatternJump), keeping loop counters intact across breaks/jumps. See `packages/tracker-playback/src/engine.ts`.
...
- Macro routing: `MacroControls` now includes per-macro routing rows (target, port, amount knob) stored in `macro-store`. A `connectMacro` worklet message fans out to all voices and calls `AudioEngine::connect_macro`, which now removes any existing route for the same macro/target and treats `amount <= 0` as removal. Routes are in-memory only and re-applied on layout changes; rebuild WASM if `npm run build:wasm` previously failed (needs wasm-bindgen install permissions).
...
- Tracker song save/load: tracker store can now serialize/restore full songs (patterns, sequence, slots, song patches, metadata). UI gets Load/Save buttons that use the File System Access API when available (or download/input fallback) to read/write `.cmod` JSON and replace the current song state, then resync the song bank and playback.
- Added minimal File System Access API type shims (`SaveFilePickerOptions`, `OpenFilePickerOptions`, `FileSystemFileHandle`) in `TrackerPage.vue` so vue-tsc passes when the DOM lib doesn’t include them.
- Tracker volume column: third column now edits per-step volume in hex (0–FF). Type a hex digit in the volume column to set the first nibble; type another to complete the byte and advance to the next step. Volume is scaled to velocity (0–127) and drives the per-voice `gain_{voice}` parameter in InstrumentV2.
- Note key mappings are now restricted to the note column only, so numeric hex digits (e.g., 2/3) work when editing volume nibbles.
...
- Playback builder now ramps once per range: start row sets the macro value and attaches a single ramp to the end row; interior rows avoid emitting macro steps so the ramp isn’t reset.
...
### MOD import sampler gain default (2025-05)
...
### Tracker effect isolation (2025-05)
...
- **Regression guard (2026-03):** Precomputing tone porta targets across the song must not retarget continuation rows to a later note. The bug: the backward “next-note” scan assigned 3xx rows without notes to the next note in the sequence, so slides snapped back toward that future note after the first tick. Fix: walk tracks forward and let 3xx rows without notes inherit the last tone-porta note target only; continuations keep sliding toward the original target. File: `packages/tracker-playback/src/engine.ts::precomputeTonePortaTargets`.
...
### Tracker FT2-style effect engine (2025-12)

- The tracker playback core in `packages/tracker-playback` implements a FastTracker 2-style per-tick effect processor (`effect-processor.ts`) plus a song-level scheduler (`engine.ts`). Effects like arpeggio, portamento (1/2/3xx + fine E1/E2), vibrato (4xx/Uxy), tremolo (7xy), volume/panning slides (Axy/Pxy), retrigger/tremor (Rxy/Txy), note cut/delay/key-off (ECx/EDx/Kxx), set volume (Cxx) and set pan (8xx/E8x) are already wired through to the SongBank via `ScheduledPitchHandler`, `ScheduledVolumeHandler`, and `ScheduledRetriggerHandler`.
- Some FT2 commands are parsed but still need dedicated engine/host wiring: `sampleOffset` (9xx), global volume commands (`setGlobalVol` Gxx / `globalVolSlide` Hxy), and pattern-delay/loop control (`patDelay` EEx, `patLoop` E6x, `setFinetune` E5x, `glissandoCtrl` E3x, and `retrigger` E9x via `extEffect`) currently surface only as effect metadata. These will matter for high-fidelity .XM/.MOD import and should be implemented either in the playback engine (flow control) or via SongBank/InstrumentV2 hooks (global gain, sampler offset, finetune).
- ProTracker `.MOD` import currently maps effect nibbles 0–F and `Exy` into the FT2-style engine via `modCellToTrackerEntry` → `parseEffectCommand`. Core ProTracker commands (0xy arpeggio, 1/2xx portamento, 3xx tone portamento, 4xx vibrato, 5/6xx combo porta/vibrato + vol slide, 7xy tremolo, Axy volume slide, Bxx position jump, Cxx set volume, Dxx pattern break with decimal row, Fxx speed/tempo, and E1/E2/E4/E7/EC/ED fine slides + waveforms + note cut/delay) are all recognized and have working FT2-style implementations in `effect-processor.ts`.
- There are a few ProTracker-specific gaps/mismatches to be aware of when importing old Amiga `.MOD`s:
  - `9xx` sample offset is parsed as `EffectType.sampleOffset` but has no runtime behavior yet; implementing this will require a sampler start-offset hook (per-note) in `TrackerSongBank`/`InstrumentV2` and likely an explicit sampler start-position control in the Rust sampler node.
...
- Pool-scoped *asset* messages follow the same rule as creation (2026-09-22, arch review N1, `.ai/plan-arch-fix1.md`). A pooled slot owns a **private** `AudioEngine` (`getOrCreateInstrumentSlot`). `audioEngines[]` are the legacy editor engines and are not rendered while any slot exists. So `deleteNode`, `importImpulseWaveform`, `exportSampleData` and `exportConvolverData` route by `instrumentId` through `getTargetEngines`, never by engine slice and never `audioEngines[0]`. `PooledInstrument.deleteNode` posts `{deleteNode, nodeId, instrumentId}`, and the worklet replies with that slot's layout. Export correlation ids name the instrument, because slots share node ids (every convolver is `10003`) and every instrument on a worklet listens on the one port. Real-bundle coverage: `src/tests/synth-worklet-pooled-scoping.test.ts`, which drives the built worklet and real wasm via `src/tests/helpers/synth-worklet-harness.ts`.
- Committed artifacts are gated (arch review N3). `src/tests/artifact-freshness.test.ts`, and `npm run check:artifacts`, fail when `public/worklets/*.js` differ from an in-memory esbuild of their sources, or when `public/wasm` no longer matches the Rust inputs recorded in `public/wasm/SOURCE_HASH.json` (written by `build-wasm.cjs`). After touching `src/audio/worklets/**` or anything they import, run `npm run build:worklets` and commit the bundle. After touching Rust, run `npm run build:wasm` and commit `public/wasm/*` including `SOURCE_HASH.json`.
...
## SID player: every register against GT's playroutine (S5.19)

- Compare all 25 registers, not the control byte: `.ai/sid-oracle/cmp_all_regs.py` (+ `side.py`, `run_gt_wide.sh`; the oracle harness takes `SUB`/`MULT` env vars). Corpus, subsong 0, 4000 frames: 1 330 563 mismatching register-frames and no exact song before; 1 130 in one song after (83/84 exact). Other subsongs (18) and 15 000 frames: exact, bar `game_tune` after its loop (the importer's known `loop-transpose`). The 2x songs at `MULT=2`: exact (GT starts 11 frames after us there, not 5).
- What GT does that the player did not (all in `player.rs`, pinned in `tests_s519.rs`): the hard restart's $40/$80 flags come from the NEXT row's instrument (GT switches instrument as it reads the row); channels start on instrument 1; GT's default `optimizepulse` skips the pulse table on the row-read (gate-timer) frame and on tick 0 of a pattern's last row, and a note's frame skips wave table, effects and pulse; the filter table runs and $15-$18 are latched at the top of the frame (a note's or command's filter change is heard a frame later); a note leaves the pitch to the wave table (the note frame keeps the old frequency); pulse and filter tables walk as GT's (a jump lands on data, a jump-to-a-jump is a width row, `00` stalls the pulse table, the cutoff byte wraps, B 00 stops the table and its sweep); all wave-table commands $F1-$FD; fine slide speeds ($8000+) for commands 1-3; slides wrap; D $10+ ignored; the pulse low bit is written clear.
- GT-style instruments (the importer's neutral waveform 0) follow GT's first-frame byte: it is held as the waveform until the table sets one, $00 leaves waveform and gate alone (a fresh channel's gate is shut), $FE/$FF set the gate only; the neutral filter leaves the routing alone. Instruments with a waveform of their own keep the old first-frame behaviour.
- Doc: `SidInstrument.noGateOff` (GT's $40, ASID gate byte bit 6) keeps the timer; the importer no longer reports `no-gateoff`.
- Left: "Ballad" uses fine vibrato shift $42; GT's editor (C on x86) shifts by 66 mod 32 = 2, its C64 player (player.s) and ours by 66 (= 0). Past a filter table's stored rows ours stops; GT would read zero rows (cutoff 0 every frame).
