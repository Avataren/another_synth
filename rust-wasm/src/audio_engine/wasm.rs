...
impl AudioEngine {
    ...
    pub fn init_with_patch(&mut self, patch_json: &str) -> Result<usize, JsValue> {
        ...
        if let (Some(sampler_id_str), Some(filter_id_str)) =
            (find_node_id(canonical_voice, "sampler"), find_node_id(canonical_voice, "filter"))
        {
            match (NodeId::from_string(&sampler_id_str), NodeId::from_string(&filter_id_str)) {
                (Ok(sampler_node_id), Ok(filter_node_id)) => {
                    if let Some(voice0) = self.voices.get(0) {
                        ...
                        for (_key, conn) in all_conns.into_iter() {
                            if conn.from_node == sampler_node_id
                                || conn.to_node == sampler_node_id
                            {
                                log_console(&format!(
