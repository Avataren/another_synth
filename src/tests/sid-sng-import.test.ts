...
import { gtSongHintsFromName, importGtSong, looksLikeGtSong, type GtSongImport, type SidDoc } from 'src/audio/tracker/sid-doc';
...
function gts5(spec: Gts5Spec = {}): Uint8Array {
  const out: number[] = [...(spec.magic ?? 'GTS5').split('').map((c) => c.charCodeAt(0))];
  out.push(...(spec.name ?? text('Song', 32)), ...text('Author', 32), ...text('(C)', 32));
  const subtunes = spec.subtunes ?? [[[0x00, 0xff, 0x00], [0x00, 0xff, 0x00], [0x00, 0xff, 0x00]]];
  out.push(subtunes.length);
  for (const lists of subtunes) for (const data of lists) out.push(data.length - 1, ...data);
  const instruments = spec.instruments ?? [[0x09, 0x00, 0, 0, 0, 0, 0, 0x02, 0x09, ...text('Lead', 16)]];
  out.push(instruments.length);
  for (const ins of instruments) out.push(...ins);
  for (const table of spec.tables ?? [[], [], [], []]) {
    out.push(table.length, ...table.map((r) => r[0]!), ...table.map((r) => r[1]!));
  }
...
describe('GTS5 (readme §6.1)', () => {
  it('reads the header texts at +4/+36/+68 up to their first NUL, and the defaults a .sng has no field for', () => {
    ...
    expect([doc.songName, doc.author, doc.copyright]).toEqual(['Alien', 'Author', '(C)']);
    expect([doc.tempo, doc.speedMultiplier, doc.chipModel, doc.channels]).toEqual([6, 1, '8580', 3]);
  });
  ...
  it('maps notes $60-$BC to 1-93, $BD rest to 0, $BE key off to 126, $BF key on to 127; the $FF row ends the pattern', () => {
    ...
    expect(doc.patterns[0]!.rows.map((r) => r.note)).toEqual([1, 93, 0, 126, 127]);
    expect(doc.patterns[0]!.rows[0]).toEqual({ note: 1, instrument: 0, command: 0xf, param: 5 });
  });

  it('maps an instrument\'s 25 bytes (§6.1.3) and the HR/Gate Timer bits (§3.3)', () => {
    const ins = (gate: number) => [0x1a, 0xb2, 1, 1, 1, 1, 0x07, gate, 0x09, ...text('X', 16)];
    const tables: Gts5Spec['tables'] = [[[0x41, 0x00]], [[0x88, 0x00]], [[0x90, 0xf1]], [[0x03, 0x40]]];
    const { doc, notes } = ok(importGtSong(gts5({ instruments: [ins(0x02), ins(0x83), ins(0x42)], tables })));
    expect(doc.instruments[0]).toEqual({
      name: 'X', attack: 1, decay: 0xa, sustain: 0xb, release: 2, waveform: 0, pulseWidth: 0,
      filter: { enabled: false, cutoff: 0, resonance: 0, mode: 0 }, firstWave: 9, gateTimer: 2, hardRestart: true,
      vibratoDelay: 7, wavePtr: 1, pulsePtr: 1, filterPtr: 1, speedPtr: 1,
    });
    expect([doc.instruments[1]!.gateTimer, doc.instruments[1]!.hardRestart]).toEqual([3, false]);
    // $40 (no gate-off) has no doc flag: timer 0, reported.
    expect([doc.instruments[2]!.gateTimer, doc.instruments[2]!.hardRestart]).toEqual([0, true]);
  ...
  it('pads a table an instrument points past with blank rows, and says so', () => {
    const { doc, notes } = ok(importGtSong(gts5({ instruments: [[0, 0, 3, 0, 0, 0, 0, 0, 0, ...text('', 16)]], tables: [[[0x41, 0]], [], [], []] })));
    expect(doc.tables.wave).toEqual([{ left: 0x41, right: 0 }, { left: 0, right: 0 }, { left: 0, right: 0 }]);
    expect(notes).toEqual([{ kind: 'table-padded', message: 'instrument 1: its wave pointer 3 is past the table\'s 1 rows; blank rows added' }]);
  });
  ...
  it('plays orderlist commands as state: a transpose holds, a repeat applies to the next pattern, R0 is 16, the restart byte names an entry', () => {
    ...
    const { doc, notes } = ok(importGtSong(gts5({ subtunes: [three], patterns: [[[0x60, 0, 0, 0]], [[0x61, 0, 0, 0]]] })));
  ...
  it('reports a loop whose running transpose differs from the first pass (the doc keeps the first pass)', () => {
    ...
    const { doc, notes } = ok(importGtSong(gts5({ subtunes: [[list, [0x00, 0xff, 0x00], [0x00, 0xff, 0x00]]], patterns: [[[0x60, 0, 0, 0]], [[0x60, 0, 0, 0]]] })));
    expect(doc.subsongs[0]!.orderlists[0]!.entries.map((e) => e.transpose)).toEqual([0, 3]);
    expect(notes.map((n) => n.kind)).toEqual(['loop-transpose']);
  });

  it('refuses, whole and with the true reason, what is not a readable song', () => {
    expect(refusal(gts5({ magic: 'GTS4' }))).toMatch(/^it is a GoatTracker 2 beta song \(GTS4\); only GTS5/);
    expect(refusal(gts5({ magic: 'GTSX' }))).toBe('it is not a GoatTracker song (no GTS5 or GTS! identification string)');
    expect(refusal(new Uint8Array(0))).toBe('it is not a GoatTracker song (no GTS5 or GTS! identification string)');
    const whole = gts5();
    expect(refusal(whole.subarray(0, 60))).toBe('the file ends inside the song header');
    expect(refusal(whole.subarray(0, 103))).toBe("the file ends inside subtune 0 channel 1's orderlist");
    expect(refusal(whole.subarray(0, 120))).toBe('the file ends inside instrument 1');
    expect(refusal(whole.subarray(0, whole.length - 3))).toBe('the file ends inside pattern 0');
    expect(refusal(gts5({ trailing: [0] }))).toBe('1 bytes follow the last pattern');
    expect(refusal(gts5({ subtunes: [] }))).toBe('it declares 0 subtunes; a song has 1-32');
    expect(refusal(gts5({ instruments: Array.from({ length: 64 }, () => Array(25).fill(0) as number[]) }))).toBe('it declares 64 instruments; a song has at most 63');
    expect(refusal(gts5({ patterns: [Array.from({ length: 129 }, () => [0xbd, 0, 0, 0])] }))).toBe('pattern 0 has 129 rows; a pattern has at most 128');
    expect(refusal(gts5({ patterns: [[[0x5f, 0, 0, 0]]] }))).toBe('pattern 0 row 0: note byte $5F is not a GoatTracker note');
    expect(refusal(gts5({ patterns: [[[0x60, 2, 0, 0]]] }))).toBe('pattern 0 row 0: instrument 2 does not exist (the song has 1)');
    expect(refusal(gts5({ patterns: [[[0x60, 0, 0x10, 0]]] }))).toBe('pattern 0 row 0: command byte $10 is not 0-F');
    const lists = (first: number[]) => [[first, [0x00, 0xff, 0x00], [0x00, 0xff, 0x00]]];
    expect(refusal(gts5({ subtunes: lists([0x00, 0x00, 0x00]) }))).toBe("subtune 0 channel 1's orderlist has no RST endmark");
    expect(refusal(gts5({ subtunes: lists([0x00, 0xff, 0x01]) }))).toBe("subtune 0 channel 1's orderlist restarts at byte 1, past its end");
    expect(refusal(gts5({ subtunes: lists([0x01, 0xff, 0x00]) }))).toBe("subtune 0 channel 1's orderlist plays pattern 1, but the song has 1");
    expect(refusal(gts5({ subtunes: lists([0x00, 0xf2, 0xff, 0x00]) }))).toBe("subtune 0 channel 1's orderlist ends with a command, not a pattern (readme §3.1)");
    expect(refusal(gts5({ subtunes: lists([0xf2, 0xff, 0x00]) }))).toBe("subtune 0 channel 1's orderlist plays no pattern");
  });

  it('refuses a dual-SID (6-orderlist) song with that reason, not as corrupt bytes', () => {
    const six = Array.from({ length: 6 }, () => [0x00, 0xff, 0x00]);
    expect(refusal(gts5({ subtunes: [six] }))).toBe('it is a dual-SID (6-channel) song; dual SID is not supported yet (plan-sid-tracking.md S7)');
  });
...
describe('the hints a file name carries (a .sng stores neither)', () => {
  ...
  it('are applied to the doc', () => {
    const { doc } = ok(importGtSong(gts5(), gtSongHintsFromName('x_6581_3x.sng')));
...
function gt1(instruments: { h: number[]; name?: string; wave: number[][] }[], patterns: number[][][], filterTable?: number[][]): Uint8Array {
  const out: number[] = [...'GTS!'.split('').map((c) => c.charCodeAt(0)), ...text('', 96), 1];
  for (let c = 0; c < 3; c++) out.push(2, 0x00, 0xff, 0x00);
  for (let i = 0; i < 31; i++) {
    const ins = instruments[i] ?? { h: [0, 0, 0, 0, 0, 0, 0], wave: [[0, 0], [0xff, 0]] };
  ...
  if (filterTable) {
    const ft = Array.from({ length: 64 }, (_, i) => filterTable[i] ?? [0x80, 0x0f, 0, 0]);
    out.push(...ft.flat());
  }
  return Uint8Array.from(out);
}
...
describe('GoatTracker 1 (GTS!), converted', () => {
  const lead = { h: [0x09, 0x00, 0x80, 0x00, 0x00, 0x00, 0x01], name: 'Lead', wave: [[0x41, 0x00], [0xff, 0x00]] };
  const sweep = { h: [0x00, 0xf0, 0x20, 0x18, 0x20, 0x80, 0x00], name: 'Sweep', wave: [[0x81, 0xc8], [0x41, 0x00], [0xff, 0x02]] };
  const rows = [
  ...
  it('keeps notes and instruments, converts the commands (readme §3.4.4 for vibrato and portamento), trims unused instruments', () => {
    ...
    expect(doc.tables.speed[r[2]!.param - 1]).toEqual({ left: 0x03, right: 0x40 });
    expect(r[3]).toMatchObject({ command: 1 });
    expect(doc.tables.speed[r[3]!.param - 1]).toEqual({ left: 0x00, right: 0x20 });
    expect(r[4]).toEqual({ note: 0, instrument: 0, command: 3, param: 0 });
    expect(r[5]).toEqual({ note: 0, instrument: 0, command: 0xf, param: 0x85 });
    expect(r[6]).toMatchObject({ command: 0xa, param: doc.instruments[0]!.filterPtr });
  ...
  it('turns the inline wavetables, the pulse sweep and the filter table into GT2 tables', () => {
    ...
    const [a, b] = doc.instruments as [SidDoc['instruments'][0], SidDoc['instruments'][0]];
    expect(doc.tables.wave.slice(a.wavePtr - 1, a.wavePtr + 1)).toEqual([{ left: 0x41, right: 0 }, { left: 0xff, right: 0 }]);
    // A jump inside an instrument's own program is relocated.
    expect(doc.tables.wave.slice(b.wavePtr - 1, b.wavePtr + 2)).toEqual([{ left: 0x81, right: 0xc8 }, { left: 0x41, right: 0 }, { left: 0xff, right: b.wavePtr + 1 }]);
    // Static width $800, then stop.
    expect(doc.tables.pulse.slice(a.pulsePtr - 1, a.pulsePtr + 1)).toEqual([{ left: 0x88, right: 0x00 }, { left: 0xff, right: 0 }]);
    ...
    expect([a.firstWave, a.gateTimer, a.hardRestart, a.vibratoDelay, a.speedPtr]).toEqual([9, 2, true, 0, 0]);
  });
