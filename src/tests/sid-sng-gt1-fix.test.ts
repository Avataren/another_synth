...
function gt1(
  ...
  if (filter)
    for (let i = 0; i < 64; i++) out.push(...(filter[i] ?? [0, 0, 0, 0]));
...
const row = (note: number, instrument: number, cmd: number, param: number) => [
  note,
  (instrument << 3) | cmd,
  param,
];
...
const slice = (t: Tables[keyof Tables], ptr: number, n: number) =>
  t.slice(ptr - 1, ptr - 1 + n);
...
describe('GT1 filter table (gsong.c:602-669)', () => {
  it('a set row: byte 1 bits 4-6 are the passband, byte 0 is resonance|channels as is (gsong.c:621-625)', () => {
    ...
    const { doc, notes } = ok(
      importGtSong(
        gt1([lead([0x09, 0, 0, 0, 0, 0, 1])], [[row(0x30, 1, 0, 0)]], f),
      ),
    );
  ...
  it('converts rows 1..numfilter in order, falls through on next = c+1, maps all-zero rows to the current end, pads what points there', () => {
    ...
    expect(doc.instruments[0]!.filterPtr).toBe(1);
    expect(
      doc.patterns[0]!.rows.slice(1).map((r) => [r.command, r.param]),
    ).toEqual([
      [0xa, 11],
      [0xa, 0],
      [0xa, 4],
  ...
  it('guards the reads GoatTracker makes out of bounds: a pointer or next row past 63 is 0, reported', () => {
    ...
    const song = gt1(
      [lead([0x09, 0, 0, 0, 0, 0, 0x50]), lead([0x09, 0, 0, 0, 0, 0, 1], 'B')],
      [[row(0x30, 1, 0, 0), row(0x30, 2, 5, 0x41)]],
      f,
    );
    const { doc, notes } = ok(importGtSong(song));
  ...
  it('without a filter table, a zero filter pointer is still A00; a non-zero one is dropped and reported', () => {
    const { doc, notes } = ok(
      importGtSong(
        gt1(
          [lead([0x09, 0, 0, 0, 0, 0, 2])],
          [[row(0x30, 1, 5, 0), row(REST, 0, 5, 3)]],
        ),
      ),
    );
...
describe('GT1 commands 6 and 7 (gsong.c:574-590, 694-699)', () => {
  it('keeps command 6 (set SR) with its parameter', () => {
    const { doc, notes } = ok(
      importGtSong(
        gt1([lead()], [[row(0x30, 1, 6, 0x00), row(REST, 0, 6, 0x3a)]]),
      ),
    );
    expect(doc.patterns[0]!.rows.map((r) => [r.command, r.param])).toEqual([
      [6, 0x00],
...
describe('GT1 arpeggio (gsong.c:700-803)', () => {
  ...
  it('clones the instrument per (instrument, param): its wave lefts, then X, Y, 0 looping; bit 7 is a 1-frame delay per step', () => {
    ...
    const r = doc.patterns[0]!.rows.map((x) => [
      x.note,
      x.instrument,
      x.command,
      x.param,
    ...
    expect(doc.patterns[1]!.rows[0]).toEqual({
      note: 0x31,
      instrument: 0,
      command: 0,
      param: 0,
    ...
    expect({ ...c3!, name: '', wavePtr: 0 }).toEqual({
      ...doc.instruments[0]!,
      name: '',
      wavePtr: 0,
    });
  ...
  it('a name of 13+ characters is not extended (gsong.c:769)', () => {
    const { doc } = ok(
      importGtSong(
        gt1([{ ...wavy, name: 'ThirteenChars' }], [[row(0x30, 1, 0, 0x04)]]),
      ),
    );
    expect(doc.instruments.map((i) => i.name)).toEqual([
      'ThirteenChars',
  ...
  it('clones start after the highest instrument a pattern uses, overwriting an unused one (gsong.c:598, 763-766)', () => {
    const { doc, notes } = ok(
      importGtSong(
        gt1([wavy, { ...bare, name: 'Unused' }], [[row(0x30, 1, 0, 0x04)]]),
      ),
    );
  ...
  it('when the 63 instrument slots run out, the row gets command 8 with the program', () => {
    ...
    expect(r.slice(1, 33).map((x) => x.instrument)).toEqual(
      Array.from({ length: 32 }, (_, k) => 32 + k),
    );
    expect(r[33]).toMatchObject({ instrument: 1, command: 8 });
    expect(slice(doc.tables.wave, r[33]!.param, 6).map((x) => x.left)).toEqual([
      0x41, 0x21, 0, 0, 0, 0xff,
    ]);
    expect(
      slice(doc.tables.wave, r[33]!.param + 2, 2).map((x) => x.right),
    ).toEqual([2, 1]);
  });

  it('when the wave table cannot hold the program, the row is left as command 0 param 0 and reported', () => {
    const long = (base: number): Gt1Instrument => ({
      h: [0x09, 0, 0, 0, 0, 0, 0],
      name: 'Long',
      wave: [
        ...Array.from({ length: 124 }, (_, k) => [0x41, base + k]),
        [0xff, 0],
      ],
    });
    const song = gt1(
      [long(0), long(1)],
      [[row(0x30, 1, 0, 0x37), row(0x30, 2, 0, 0), row(0x30, 1, 0, 0x37)]],
    );
    const { doc, notes } = ok(importGtSong(song));
    expect(
      doc.patterns[0]!.rows.map((x) => [x.instrument, x.command, x.param]),
    ).toEqual([
      [1, 0, 0],
      [2, 0, 0],
      [1, 0, 0],
...
describe('GT1 pulse (gsong.c:379-538, 816-830)', () => {
  const pulse = (bytes: number[]) => {
    const { doc, notes } = ok(
      importGtSong(
        gt1([lead([0x09, 0x00, ...bytes, 0])], [[row(0x30, 1, 0, 0)]]),
      ),
    );
  ...
  it('bit 0 of the start byte is "no hard restart", and not part of the width', () => {
    ...
    expect(pulse([0x83, 0x00, 0x00, 0x00]).table[0]).toEqual({
      left: 0x88,
      right: 0x20,
    });
  ...
  it('speed 0 holds the width', () => {
    expect(pulse([0x40, 0x00, 0x10, 0x80]).table).toEqual([
...
describe('GT1 notes and wavetable bytes', () => {
  it('rests note bytes $5D and $60-$9E silently, as GoatTracker does (gsong.c:559-560)', () => {
    const { doc, notes } = ok(
      importGtSong(
        gt1(
          [lead()],
          [[row(0x5d, 1, 0, 0), row(0x60, 0, 0, 0), row(0x9e, 0, 0, 0)]],
        ),
      ),
    );
  ...
  it('rests $9F-$FE too, reporting that GoatTracker makes them its pattern end or wraps them to non-notes', () => {
    const { doc, notes } = ok(
      importGtSong(
        gt1(
          [lead()],
          [[row(0x9f, 1, 0, 0), row(0xa0, 0, 0, 0), row(0xfe, 0, 0, 0)]],
        ),
      ),
    );
    expect(doc.patterns[0]!.rows.map((r) => r.note)).toEqual([0, 0, 0]);
  ...
  it('ORs wavetable lefts $08-$0F with $E0 (pre-2.18 delays, gsong.c:396-397)', () => {
    ...
    expect(
      slice(doc.tables.wave, doc.instruments[0]!.wavePtr, 4).map((r) => r.left),
    ).toEqual([0x07, 0xe8, 0xef, 0x10]);
  });
});
  ...
  const load = (name: string) =>
    ok(
      importGtSong(
        ...
        gtSongHintsFromName(name),
      ),
    );

  it('the 16 "no hard restart" instruments (pulse bit 0) of three files import with hard restart off', () => {
    const off = (name: string) =>
      load(name).doc.instruments.filter((i) => !i.hardRestart).length;
    // 2 + 5 + 9 in the files' slots; arpeggio clones copy the flag (1 in wod, 4 in b.o.f.h.).
    expect(off('cadaver/galwaytest.sng')).toBe(2);
    expect(off('shinobi/wod.sng')).toBe(6);
    expect(off('yehar/b_o_f_h_ingame_death_victory.sng')).toBe(13);
    expect(off('cadaver/tarantula.sng')).toBe(0);
  });

  it('the two command-6 rows keep command 6 00', () => {
    expect(
      load('aeuk/metal_warrior_4_streets.sng').doc.patterns[40]!.rows[0],
    ).toMatchObject({ command: 6, param: 0 });
    expect(
      load('cadaver/tarantula.sng').doc.patterns[2]!.rows[0],
    ).toMatchObject({ command: 6, param: 0 });
  });
});
