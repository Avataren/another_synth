 ...
 * The 7 `.hvl` files are library-level only (`ahx-writer-corpus.test.ts`); the
 * exporter refuses them.
 */
...
describe('the AHX exporter over the demo corpus, through the store', () => {
  ...
  it('refuses all 7 .hvl songs as unavailable, with the HVL reason', () => {
    let checked = 0;
    for (const { name, bytes } of corpus.filter((f) => !name_isAhx(f.name))) {
      const song = snapshotEditorSong(openInEditor(bytes));
      const verdict = ahxExporter.check(song);
      expect(verdict, name).toEqual({ ok: false, reason: 'HVL (.hvl) songs are not exported yet: only .ahx songs.' });
      expect(() => ahxExporter.serialize(song), name).toThrow(/HVL/);
      checked++;
    }
