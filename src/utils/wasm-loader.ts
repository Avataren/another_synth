...
export async function loadWasmModule(path: string, memory: WebAssembly.Memory): Promise<WasmExports> {
    ...
    const { instance } = await WebAssembly.instantiateStreaming(response, {
        env: {
            ...
            abort(message: number, fileName: number, lineNumber: number, columnNumber: number) {
                console.error(`Abort called at ${message}, ${fileName}:${lineNumber}:${columnNumber}`);
            },
        },
    });
